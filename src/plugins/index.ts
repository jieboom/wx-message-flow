...
// 获取插件名称
export const getPluginName = (type:string,subType?:string):string =>  `${type}${subType ?`\\${subType}`:'' }`

// 判断插件库是否已有该插件
export const hasPlugin = (getPluginName:string) => plugins.has(getPluginName)

// 根据插件类型返回插件
export const getPlugin = (type:string,subType?:string): React.FunctionComponent<Message<MSomeBodyType>> => {
    console.log(plugins);
...
// 向插件库注入插件
export const setPlugin = (component:  React.FunctionComponent<Message<MAllBodyType>>,type:string,subType?:string) => {
   const pluginName = getPluginName(type,subType)
